...
import { parseSettingsRootProjectName, toUniqueSettingsRootProjectName } from '../common/iexec/spring.js';
...
export async function patchSettingsDotGradle(dir, setup) {
    ...
    /** @todo what if semver is not computed ? */
    assert(pkgDir.pkgArgCommitInfo.semver, 'semver not computed');
    const version = pkgDir.pkgArgCommitInfo.semver.version;
    ...
    const settingsLines = str.split('\n');
    for (let i = 0; i < settingsLines.length; ++i) {
        const line = settingsLines[i].trim();
        if (line.indexOf('rootProject.name') < 0) {
            continue;
        }
        const rootProjectName = parseSettingsRootProjectName(line);
        if (!rootProjectName) {
            break;
        }

        // A unique name is required to fix the case when : 
        // - 2 projects have the same root name BUT 2 different versions
        // - both projects are included in the same vscode workspace
        // - 'Duplicate root element' error in vscode + Red Hat Java Language Server

        const uniqueRootProjectName = toUniqueSettingsRootProjectName(rootProjectName, version);
        settingsLines[i] = `rootProject.name = '${uniqueRootProjectName}'`;

        str = settingsLines.join('\n');
