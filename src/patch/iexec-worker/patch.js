...
function patchFail(strict) {
    return fail(
        new CodeError('iexec-worker patch failed.', ERROR_CODES.PKGMGR_ERROR),
        strict);
}
...
export async function patch(dir, setup, options = { strict: false }) {
    ...
    try {
        ...
        await patchBuildInfoIfNeeded(dir, setup, 'src/main/java/com/iexec/worker/utils/version/VersionService.java');
