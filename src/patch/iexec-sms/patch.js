...
function patchFail(strict) {
    return fail(
        new CodeError('iexec-sms patch failed.', ERROR_CODES.PKGMGR_ERROR),
        strict);
}
...
export async function patch(dir, setup, options = { strict: false }) {
    ...
    try {
        ...
        //     throw null;
        // }
        // ok = await patchBuildDotGradle('iexec-sms-library/build.gradle', dir, setup);
        // if (!ok) {
        //     throw null;
        // }

        await patchBuildInfoIfNeeded(dir, setup, 'src/main/java/com/iexec/sms/utils/version/VersionService.java');
